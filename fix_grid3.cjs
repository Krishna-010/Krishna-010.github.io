...
let cards = [];
const cardRegex = /<div class="project-item-card"[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/g;

let replacedHTML = html.replace(cardRegex, (match) => {
  let m = match.match(/onclick="openProjectDetail\((\d+)\)"/);
  if (m) {
    let oldIdx = parseInt(m[1], 10);
    cards.push({ oldIdx, cardStr: match });
  }
...
let sortedCardsIter = cards[Symbol.iterator]();

replacedHTML = replacedHTML.replace(/###CARD_PLACEHOLDER###/g, () => {
  return sortedCardsIter.next().value.cardStr;
});

fs.writeFileSync('index.html', replacedHTML);
console.log("Grid replacement complete. Cards replaced:", cards.length);
