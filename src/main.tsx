      ...
      if (panelTab === targetTabId) {
        ...
        if (!skipAnimation) {
          ...
          panel.style.animation =
            "zoomInPanel 0.45s cubic-bezier(0.16, 1, 0.3, 1) forwards";
        }
      } else {
...
function updateProjectCarousel() {
  ...
  details.forEach((el) => {
    ...
    if (elIndex === currentProjectIndex) {
      ...
      void el.offsetWidth;
      el.style.animation =
        "zoomInPanel 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards";
    } else {
...
window.nextProjectImage = function (projectIndex) {
  const container = document.querySelector(
    `.project-image-carousel[data-project-id="${projectIndex}"]`,
  );
  if (!container) return;
  ...
  if (projectImageIndices[projectIndex] === undefined)
    projectImageIndices[projectIndex] = 0;
  projectImageIndices[projectIndex] =
    (projectImageIndices[projectIndex] + 1) % totalImages;
...
window.prevProjectImage = function (projectIndex) {
  ...
  if (projectImageIndices[projectIndex] === undefined)
    projectImageIndices[projectIndex] = 0;
  projectImageIndices[projectIndex] =
    (projectImageIndices[projectIndex] - 1 + totalImages) % totalImages;
  updateProjectImageCarousel(projectIndex, container);
};
